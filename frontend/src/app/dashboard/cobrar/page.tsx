...
import {
    Plus, TrendingUp, AlertCircle, Clock,
    CheckCircle, X, Loader2, ChevronDown, Check
} from 'lucide-react'
import { cn } from '@/lib/utils'
import { cxcService } from '@/services/cxcService'
import { CuentaPorCobrarDto, GetCuentasPorCobrarResult } from '@/types/cxc'
...
export default function CxCPage() {
    ...
    const [data, setData] = useState<GetCuentasPorCobrarResult | null>(null)
    ...
    const cargar = useCallback(async () => {
        setLoading(true)
        try {
            const result = await cxcService.getCuentas(
    ...
    const handleGuardar = async () => {
        if (!form.nombreContacto || !form.concepto || !form.montoTotal) return
        setGuardando(true)
        setError(null)
        try {
            await cxcService.crearCuenta({
    ...
    const erroresCxC = {
        ...
        montoTotal: (() => {
            ...
            if (!/^\d+(\.\d{1,2})?$/.test(form.montoTotal)) return 'Solo 2 decimales'
            return null
        })(),
        fechas: (() => {
    ...
    const cxcValido = !Object.values(erroresCxC).some(Boolean)
    const [touchedCxC, setTouchedCxC] = useState<Record<string, boolean>>({})
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                        Lo que te deben pacientes y terceros
                    </p>
                </div>
                <button
                    onClick={() => { setForm(formInicial); setModalOpen(true) }}
                                ...
                                <div key={c.id}
                                    ...
                                    <div className="flex items-start justify-between sm:hidden">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[10px] text-gray-500 truncate">{c.concepto}</p>
                                            <p className="text-[10px] text-gray-400 mt-0.5">
                                                ...
                                                {etiqueta && (
                                                    <span className={cn('ml-1 font-medium',
                                                        c.diasVencimiento < 0 ? 'text-red-500' : 'text-orange-500')}>
                                                        · {etiqueta}
                                                    </span>
                                                )}
                                            </p>
                                    ...
                                    <div className="hidden sm:grid grid-cols-12 items-center">
                                        ...
                                        <div className="col-span-3">
                                            <p className="text-xs text-gray-600 truncate">{c.concepto}</p>
                                            {c.numeroDocumento && (
                                                <p className="text-[10px] text-gray-400 mt-0.5">
                                                    {c.numeroDocumento}
                                                </p>
                        ...
                        <div className="px-5 py-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <FormInput
                                    ...
                                    onBlur={() => setTouchedCxC(t => ({ ...t, nombreContacto: true }))}
                                    error={touchedCxC.nombreContacto ? erroresCxC.nombreContacto : null}
                                />
                                <FormSelect
                                    label="Tipo"
                            ...
                            <FormInput
                                ...
                                onBlur={() => setTouchedCxC(t => ({ ...t, concepto: true }))}
                                error={touchedCxC.concepto ? erroresCxC.concepto : null}
                            />
                            ...
                            <FormAmount
                                ...
                                onBlur={() => setTouchedCxC(t => ({ ...t, montoTotal: true }))}
                                error={touchedCxC.montoTotal ? erroresCxC.montoTotal : null}
                            />
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <FormDate
                                    ...
                                    onChange={v => {
                                        ...
                                        setTouchedCxC(t => ({ ...t, fechas: true }))
                                    }}
                                    error={touchedCxC.fechas ? erroresCxC.fechas : null}
                                />
                            </div>
                            ...
                            <FormInput
                                ...
                                onBlur={() => setTouchedCxC(t => ({ ...t, numeroDocumento: true }))}
                                error={touchedCxC.numeroDocumento ? erroresCxC.numeroDocumento : null}
                                hint="Opcional"
                            />
              ...
              border-t border-gray-100">
                  ...
                  hover:text-gray-900 transition-colors"
                            ...
                            <button
                                ...
                                disabled={guardando || !cxcValido}
                                className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium
    bg-[#1a3f7a] hover:bg-[#153468] text-white rounded-lg transition-colors
    disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {guardando ? (
                                    <><Loader2 className="w-3.5 h-3.5 animate-spin" />Guardando...</>
                                ) : (
                                    <><Check className="w-3.5 h-3.5" />Guardar cuenta</>
                                )}
                            </button>
                        </div>
                    </div>
